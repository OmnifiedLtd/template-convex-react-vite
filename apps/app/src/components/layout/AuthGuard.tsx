...
export function AuthGuard({ children }: AuthGuardProps) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (prevState.current.isAuthenticated && !currentState.isAuthenticated && !currentState.loading) {
        console.warn(
          "[Auth Debug] AuthGuard: User became unauthenticated, will redirect to /auth"
        );
        Sentry.addBreadcrumb({
          category: "auth",
